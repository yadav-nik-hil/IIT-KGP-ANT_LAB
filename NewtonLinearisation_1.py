#Q. y''' + yy'' + 1 - (y')^2 = 0, y(0) = 0, y'(0) = 0, y'(10) = 1, h = 2,0.5,0.01
...
def newtonLinearisation(Y, F, h, N):
    ...
    # make changes in ai,bi,ci and di acc to need
    ai = np.array(ai)
    a = []
    for i in range(1,N):
        ai_ = ai.copy()
    ...
    for i in range(1,N):
        di_ = di.copy()
        di_[0] = (-F[i+1]/(h*h))+(2*F[i]/(h*h))-(F[i-1]/(h*h))-(Y[i]*F[i+1]/(2*h))+(Y[i]*F[i-1]/(2*h))+(F[i]*F[i])-1
        d.append(di_)
    d = np.array(d)
